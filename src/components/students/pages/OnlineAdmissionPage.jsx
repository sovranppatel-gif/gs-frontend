...
export default function OnlineAdmissionPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setToast(
        `Application ${entry?.admissionId || ''} submitted. Waiting for Grow Skills Tech approval.`
      )
        ...
        <Panel title="Application Status">
          <div className="flex flex-wrap items-center gap-3 text-sm">
            ...
            <p className="text-slate-600">
              ...
              {isUnderReview
                ? ' is with Grow Skills Tech for verification and approval.'
                : null}
              {isApproved
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
          <p className="text-sm text-slate-600">
            {isUnderReview
              ? 'Your application is already under review. Please wait for Grow Skills Tech approval.'
              : isApproved
