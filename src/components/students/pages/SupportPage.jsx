...
import { faqs, supportTickets } from '../../../data/studentData.js'
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        <Panel title="Raise a Ticket">
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            {submitted ? (
              <p className="text-xs font-medium text-[#008C95]">Ticket submitted (demo). Support will reply soon.</p>
            ) : null}
          </form>
        </Panel>
        ...
        <Panel title="Your Tickets">
          <ul className="space-y-2">
            {supportTickets.map((t) => (
              <li key={t.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-slate-100 px-3 py-2.5 text-sm">
                <div>
        ...
        <div className="space-y-2">
          {faqs.map((f, i) => (
            <div key={f.q} className="rounded-lg border border-slate-100">
              ...
              {openFaq === i ? <p className="border-t border-slate-50 px-3 py-2.5 text-sm text-slate-600">{f.a}</p> : null}
            </div>
          ))}
        </div>
      </Panel>
