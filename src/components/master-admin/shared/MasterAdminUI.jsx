...
export function DataTable({
  ...
  return (
    <>
            ...
            <div className="space-y-2">
              {dataColumns.map((col) => (
                <div key={col.key} className="flex items-start justify-between gap-3 text-sm">
                  ...
                  <div
                    className={`min-w-0 text-right font-medium text-slate-800 ${
      ...
      {/* Desktop table */}
      <div className={`hidden md:block ${wrap ? 'overflow-x-hidden' : 'overflow-x-auto'}`}>
        <table className={`w-full text-left text-sm ${wrap ? 'table-fixed' : 'min-w-[720px]'}`}>
          <thead>
