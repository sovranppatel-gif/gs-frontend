...
const universityOptionLabel = (uni) => {
...
function findCourseByLabel(list, label) {
  const key = String(label || '').trim().toLowerCase()
  if (!key) return null
  return (
    list.find((item) => courseLabel(item).toLowerCase() === key) ||
    list.find((item) => String(item.name || '').trim().toLowerCase() === key) ||
    list.find((item) => String(item.code || '').trim().toLowerCase() === key) ||
    null
  )
}
...
function feesFromCourse(course) {
  return {
    ...
    registrationFee: String(course?.fees?.registration || '').trim(),
  }
}
...
export default function AdmissionFormPage({ editingId: editingIdProp = null, embedded = false, hideGuardian = false, onCancel = null }) {
  const navigate = useNavigate()
  const location = useLocation()
  const editingId = editingIdProp || location.state?.editingId || null
    ...
    ;(async () => {
      try {
        const [uniData, courseData] = await Promise.all([
          getUniversities(),
          getCourses({ status: 'Active' }).catch(() => ({ rows: [] })),
        ])
        if (cancelled) return
        const rows = Array.isArray(uniData.rows) ? uniData.rows : []
        setUniversities(rows.filter((row) => row.status === 'Active'))
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        setLoadedCollege(entry.college || d.universityName || '')
        setForm({
          ...emptyForm(),
          ...
          education:
            Array.isArray(d.education) && d.education.length
              ...
              : [emptyEducation()],
          documents:
            Array.isArray(d.documents) && d.documents.length
              ? d.documents.map((row) => mapDocumentRow(row))
              : [emptyDocument()],
  ...
  const selectedCourse = useMemo(
    () => findCourseByLabel(courses, form.course),
    [courses, form.course],
  )
  ...
  useEffect(() => {
    ...
    setForm((prev) => {
      ...
      if (!String(prev.registrationFee || '').trim() && fees.registrationFee) {
        next.registrationFee = fees.registrationFee
        changed = true
      }
  ...
  const courseOptions = useMemo(() => {
    const uniId = String(form.universityId || '')
    const linked = courses.filter(
      (c) =>
        String(c.universityId || '') === uniId &&
        String(c.status || 'Active') !== 'Inactive',
    )
    const labels = linked.map((item) => courseLabel(item)).filter(Boolean)
    if (form.course && !labels.includes(form.course)) {
      return [form.course, ...labels]
    }
    return labels
  }, [courses, form.universityId, form.course])
  ...
  const onChange = (key, value) => {
    setForm((prev) => {
      ...
      if (key === 'course') {
        const fees = feesFromCourse(findCourseByLabel(courses, value))
        next.totalFee = fees.totalFee
        next.registrationFee = fees.registrationFee
      }
      if (key === 'fatherName') {
  ...
  const patchDocument = (index, patch) => {
    setForm((prev) => ({
      ...prev,
      documents: prev.documents.map((row, i) =>
        i === index ? { ...row, ...patch } : row,
      ),
    }))
  }
  ...
  const handleDocumentUpload = async (index, event) => {
    ...
    const validType =
      ...
      /\.(pdf|jpe?g|png|webp|gif)$/i.test(file.name)
    if (!validType) {
      patchDocument(index, { error: 'Only PDF or image files are allowed' })
      return
    }
    if (file.size > EDU_DOC_MAX_BYTES) {
      patchDocument(index, { error: 'Document must be 400 KB or smaller' })
      return
    }

    const token = getMasterAdminToken()
    if (!token) {
      patchDocument(index, { error: 'Please log in again to upload documents' })
      return
    }

    patchDocument(index, { uploading: true, error: '' })
    try {
      const data = await uploadEducationDocument(file, token)
      patchDocument(index, {
        url: data.url || '',
  ...
  const goBack = () => {
    ...
    navigate(masterAdminDashboardPath('Admissions'))
  }
  ...
  const handlePrint = () => {
    try {
      ...
      printAdmissionForm({
        ...
        fee: displayTotalFee || '₹5,000',
        status: form.status,
        college: details.universityName,
  ...
  const handleSave = async () => {
    ...
    if (!form.course.trim()) return setError('Please select a course')
    ...
    try {
      ...
      if (editingId) {
        const updated = await updateAdmission(editingId, payload)
        setToast('Admission updated')
        setSuccessEntry(updated)
      } else {
        // generate a per-attempt idempotency key to protect against retries
        const idempotencyKey = (window.crypto && window.crypto.randomUUID && window.crypto.randomUUID()) || `${Date.now()}-${Math.random().toString(36).slice(2)}`
        const created = await createAdmission(payload, idempotencyKey)
        setToast('Admission saved')
        setSuccessEntry(created)
      }
      // Do not immediately navigate away — show confirmation for operator to act
      // allow manual continue
    } catch (err) {
      // prioritize field-level errors returned by server
      if (err?.errors && typeof err.errors === 'object') {
        const msgs = Object.entries(err.errors).map(([k, v]) => `${k}: ${v}`)
        setError(msgs.join(' | ') || err?.message || 'Save failed')
      } else {
  ...
  return (
    <section className="space-y-3">
      ...
      {successEntry ? (
        <article className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800">
          <p className="font-semibold">Admission Created Successfully</p>
          <div className="mt-2 grid grid-cols-1 gap-1 sm:grid-cols-4">
            <div><span className="text-xs text-slate-600">Admission ID</span><div className="font-medium">{successEntry.admissionId || successEntry.id}</div></div>
            <div><span className="text-xs text-slate-600">Student</span><div className="font-medium">{successEntry.applicant}</div></div>
            <div><span className="text-xs text-slate-600">Course</span><div className="font-medium">{successEntry.course}</div></div>
            <div><span className="text-xs text-slate-600">Session</span><div className="font-medium">{(successEntry.details && successEntry.details.session) || form.session}</div></div>
          </div>
          <div className="mt-3 flex gap-2">
            <PrimaryButton onClick={() => { setSuccessEntry(null); goBack(); }}>View Admission</PrimaryButton>
            <SecondaryButton onClick={() => {
              setSuccessEntry(null);
              setForm(emptyForm());
              setError('');
              setToast('Ready to create another admission')
            }}>Create Another</SecondaryButton>
            <SecondaryButton onClick={() => { handlePrint(); }}>Print / Download</SecondaryButton>
          </div>
        </article>
      ) : null}
      ...
      <div className={`flex flex-col gap-3 ${card} p-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between`}>
        <div className="flex min-w-0 flex-wrap items-center gap-2">
          <SecondaryButton onClick={goBack}>
            <ArrowLeft size={14} /> {embedded ? 'Back to Student' : 'Back to Admissions'}
          </SecondaryButton>
          {!embedded ? <div>
        ...
        <div className="flex w-full flex-wrap items-center gap-2 sm:w-auto">
          {!embedded ? <SecondaryButton onClick={handlePrint} className="flex-1 sm:flex-none">
      ...
      <Section title="Course & Session">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <label className="block">
            <span className={labelCls}>Registration No.</span>
            <input
      ...
      <Section
        ...
        action={
          <button
            type="button"
            onClick={addDocument}
            className="rounded-md border border-[#008C95]/35 px-2.5 py-1 text-[11px] font-semibold text-[#008C95] hover:bg-[#008C95]/10"
          >
            + Add document
          </button>
        }
      >
        <div className="space-y-2">
          {form.documents.map((document, index) => (
            <div key={`document-${index}`} className="grid gap-2 rounded-md border border-slate-200 p-2 sm:grid-cols-2 lg:grid-cols-5 lg:items-end">
              <label className="block">
                <span className={labelCls}>Document Type</span>
                <select value={document.type} onChange={(e) => onDocumentChange(index, 'type', e.target.value)} className={fieldCls}>
              ...
              <label className="block">
                ...
                <input value={document.name} onChange={(e) => onDocumentChange(index, 'name', e.target.value)} className={fieldCls} />
              </label>
              <label className="block">
                <span className={labelCls}>Document Number</span>
                <input value={document.number} onChange={(e) => onDocumentChange(index, 'number', e.target.value)} className={fieldCls} />
              </label>
              <div className="flex items-end">
                {document.url ? (
                  <a href={absoluteUploadUrl(document.url)} target="_blank" rel="noreferrer" className="flex h-10 w-full items-center justify-center rounded-lg bg-[#008C95] px-3 text-xs font-semibold text-white hover:bg-[#007680]">
                    {document.fileName || 'View file'}
                  </a>
                ) : (
                  <label className="flex h-10 w-full cursor-pointer items-center justify-center rounded-lg bg-[#008C95] px-3 text-xs font-semibold text-white hover:bg-[#007680]">
                    {document.uploading ? 'Uploading...' : 'Upload file'}
                    <input type="file" accept={EDU_DOC_ACCEPT} className="hidden" disabled={document.uploading} onChange={(e) => handleDocumentUpload(index, e)} />
                  </label>
                )}
              </div>
              <button type="button" onClick={() => removeDocument(index)} className="h-10 rounded-lg border border-rose-200 px-3 text-xs font-semibold text-rose-600 hover:bg-rose-50">
                Remove
              </button>
              {document.error ? <p className="text-[11px] text-rose-600 sm:col-span-2 lg:col-span-5">{document.error}</p> : null}
            </div>
          ))}
        </div>
      </Section>

      <Section title="Fees & Office">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <label className="block">
          ...
          <label className="block">
            <span className={labelCls}>Total Fee</span>
            <input
              value={displayTotalFee}
              readOnly
              tabIndex={-1}
              className={`${fieldCls} cursor-not-allowed bg-slate-50 text-slate-700`}
              placeholder={form.course ? 'No fee on this course' : 'Select a course'}
            />
          </label>
          <label className="block">
            ...
            <input
              ...
              className={fieldCls}
              placeholder={selectedCourse?.fees?.registration || '₹1,500'}
            />
          </label>
          <label className="block">
      ...
      {!hideGuardian ? <Section title="शपथ पत्र / Guardian">
      ...
      <div className="flex flex-wrap justify-end gap-2">
        ...
        <PrimaryButton disabled={saving} onClick={handleSave}>
          <Save size={14} />
          {saving ? 'Saving…' : editingId ? 'Update Student' : 'Save Admission'}
        </PrimaryButton>
      </div>
    </section>
  )
