...
export default function FacultyAssignments({ facultyId, onToast, onError }) {
  ...
  useEffect(() => {
    ...
    getUniversities()
      // New assignments should only be made against a currently active
      // university — same rule Courses/Batches/Admissions already apply to
      // their own "assign to a university" dropdowns.
      .then((data) => setUniversities((data.rows || []).filter((u) => u.status !== 'Inactive')))
