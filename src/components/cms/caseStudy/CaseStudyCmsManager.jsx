...
import {
  ...
  deleteCaseStudy,
  editCaseStudy,
  flipCaseStudyPublish,
  flipCaseStudyVisibility,
  loadCaseStudyList,
} from "../../../redux/caseStudy/caseStudySlice.js";
...
export default function CaseStudyCmsManager() {
  const dispatch = useDispatch();
  const { rows, pagination, loading, error, mutationLoading } = useSelector((state) => state.caseStudy);
  ...
  useEffect(() => {
    const timeout = window.setTimeout(() => {
      dispatch(loadCaseStudyList({ page, limit: 10, search }));
    }, 350);
  ...
  const refetch = async () => {
    await dispatch(loadCaseStudyList({ page, limit: 10, search })).unwrap();
  };

  const onSubmit = async (values) => {
    try {
      if (editingRow?._id) {
        await dispatch(editCaseStudy({ id: editingRow._id, payload: values })).unwrap();
        setToast({ type: "success", message: "Case study strip updated" });
      } else {
        await dispatch(addCaseStudy(values)).unwrap();
        setToast({ type: "success", message: "Case study strip created" });
      }
      setIsModalOpen(false);
      await refetch();
    } catch (err) {
      setToast({ type: "error", message: err || "Unable to save case study strip" });
    }
  };

  const toggleVisibility = async (row) => {
    try {
      await dispatch(flipCaseStudyVisibility(row._id)).unwrap();
  ...
  const togglePublish = async (row) => {
    try {
      await dispatch(flipCaseStudyPublish(row._id)).unwrap();
  ...
  const removeRow = async () => {
    try {
      await dispatch(deleteCaseStudy(confirmDelete._id)).unwrap();
      setToast({ type: "success", message: "Case study strip deleted" });
      setConfirmDelete(null);
      await refetch();
    } catch (err) {
      setToast({ type: "error", message: err || "Unable to delete case study strip" });
    }
  };
  ...
  const openEdit = (row) => {
    ...
    reset({
      ...
      snapshotLabel: row.snapshotLabel || "Snapshot",
      metrics: row.metrics?.length ? row.metrics : [{ value: "", label: "" }],
    ...
    <section className="space-y-3">
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-3">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Case study strip CMS</h2>
            <p className="text-sm text-slate-500">Manage highlight copy and snapshot metrics.</p>
          </div>
          <div className="flex w-full gap-2 md:w-auto">
            <input
              ...
              placeholder="Search case study content..."
          ...
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white p-5">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-lg font-semibold text-slate-900">
                {editingRow ? "Edit case study strip" : "Add case study strip"}
              </h3>
            ...
            <form className="space-y-3" onSubmit={handleSubmit(onSubmit)}>
              ...
              <label className="block text-sm text-slate-700">
                Description
                <textarea rows={4} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2" {...register("description", { required: true })} />
              </label>
              <label className="block text-sm text-slate-700">
                Snapshot card title
                <input className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2" {...register("snapshotLabel")} />
              </label>

              <div className="rounded-lg border border-slate-200 p-3">
                <div className="mb-2 flex items-center justify-between">
                  <p className="text-sm font-semibold text-slate-800">Metrics</p>
                  <button type="button" onClick={() => append({ value: "", label: "" })} className="rounded-lg border border-slate-200 px-3 py-1 text-xs">
                    Add metric
                  </button>
                </div>
                  ...
                  {fields.map((field, index) => (
                    <div key={field.id} className="grid gap-2 sm:grid-cols-[1fr_1fr_auto]">
                      <input
                        ...
                        {...register(`metrics.${index}.value`, { required: true })}
                      />
                      <input
                        placeholder="Label"
                        className="rounded-lg border border-slate-200 px-3 py-2 text-sm"
                        {...register(`metrics.${index}.label`, { required: true })}
                      />
                      <button
                        type="button"
      ...
      {confirmDelete ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/45 p-4">
          <div className="w-full max-w-md rounded-lg bg-white p-5">
            <h3 className="text-base font-semibold text-slate-900">Delete case study strip?</h3>
