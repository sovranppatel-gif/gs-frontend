...
import { FACULTY_THEME_KEY } from '../../utils/facultyHelpers.js'

const FacultyThemeContext = createContext(null)

function readStoredTheme() {
  try {
    return localStorage.getItem(FACULTY_THEME_KEY) === 'light' ? 'light' : 'dark'
  } catch {
    return 'dark'
  }
}

export function FacultyThemeProvider({ children }) {
  const [theme, setThemeState] = useState(readStoredTheme)

  useEffect(() => {
    try {
      localStorage.setItem(FACULTY_THEME_KEY, theme)
    } catch {
      /* ignore */
    }
  }, [theme])
  ...
  return <FacultyThemeContext.Provider value={value}>{children}</FacultyThemeContext.Provider>
}

export function useFacultyTheme() {
  const ctx = useContext(FacultyThemeContext)
  if (!ctx) {
    throw new Error('useFacultyTheme must be used within FacultyThemeProvider')
  }
  return ctx
}

/** Shell class tokens for faculty portal dark / light modes */
export function getFacultyShell(isDark) {
  if (isDark) {
    return {
